...
class WeChatOAuth{
    ...
    public static function getCode($redirect_uri, $state=1, $scope='snsapi_base'){
        ...
        $redirect_uri = \weChatConfig::getUrl() . $redirect_uri;
    ...
    public static function getAccessTokenAndOpenId($code){
        ...
        //构造请求微信接口的URL
        $url = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid='.\weChatConfig::getAppId().'&secret='.\weChatConfig::getAppSecret().'&code='.$code.'&grant_type='.$grant_type.'';
    ...
    public static function refreshToken($refreshToken){
        $queryUrl = 'https://api.weixin.qq.com/sns/oauth2/refresh_token?appid='.\weChatConfig::getAppId().'&grant_type=refresh_token&refresh_token='.$refreshToken;
        $queryAction = 'GET';
    ...
    public static function getUserInfo($accessToken, $openId, $lang='zh_CN'){
       ...
       // $queryUrl = 'https://api.weixin.qq.com/cgi-bin/user/info?access_token='. $accessToken . '&openid='. $openId .'&lang=zh_CN';
        $queryAction = 'GET';
        return Curl::callWebServer($queryUrl, '', $queryAction);
    }
