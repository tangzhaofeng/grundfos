...
class ActivityAwardModel extends Model
{
    protected  $table = 'xk_active_award_play_user';
     /**
     * 根据管理员id获取角色信息
     * @param $id
     */
     ...
     public function getDataByWhere($where, $size) {
        if( !request()->isAjax())
            return $this->where($where)->order('id desc')->paginate($size)->each(function($item,$index){
                        $ret = Db("member_openid_unionid")->where("openid",$item["openid"])->find();
                        $item['outh_user'] = Db("member")->where("unionid",$ret["unionid"]);
                    });
        else
            //ajax分页请求    
