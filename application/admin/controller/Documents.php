...
class Documents extends Base
{
    ...
    public function export(&$param)
    {
       if (isset($param['export']) &&  $param['export']==true) {
            $data = Db("document")->order("id desc")->field("id,customerCode,title,file_size,views,language,create_time")->select();
            ...
            $title =[ ['Document id','Customer name','Title','Occupied space','Views','Language','Create time'] ] ;
            $data = array_merge($title,$data);
            $Event = \think\Loader::controller('admin/Common');
            $Event->export_xls($data,"Document.xls");   
            exit();
        }
    }
}
