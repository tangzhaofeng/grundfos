...
class Index extends Controller
{
    ...
    function index() {
        ...
        if (input('param.question_id')){
            $param['question_id'] = input('param.question_id');
        }
        ...
        //问卷调查
        if (input('param.question_id')){
            $this->redirect($h_url.$newview.'?question_id='.input('param.question_id'));
        }
    ...
    function isRegister($member_info,$appid,$param)
    {
        ...
        if(!$obj['name']){
            ...
            if(!empty($param['question_id'])){
                ...
                $this->redirect('/glf/gelanfu-view/register/index.html?question_id='.$param['question_id']);
            }
...
//        dump('https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='.$appid.'&secret='.$appsecret);exit();
    ...
    function getQrTicket($token,$scene,$is_long)
    {
        ...
        if ($is_long == 0){
            ...
            $qrcode = '{"expire_seconds": 2592000, "action_name": "QR_STR_SCENE", "action_info": {"scene": {"scene_str": "'.$scene.'"}}}';
        }
     ...
     * 获取用户是否已经关注
     */
    function isFollow($member_id,$config)
    {
        $member = new MemberModel();
        //获取token与openid
        $access_toekn = $this->getAccessToken($config['appid'],$config['appsecret'],$config['type']);
        $openid = $member->getOpenid(['id'=>$member_id,'bu_id'=>$config['bu_id']]);
        $api = "https://api.weixin.qq.com/cgi-bin/user/info?access_token=".$access_toekn."&openid=".$openid['openid'];

        $subscribe = Curl::callWebServer($api);
